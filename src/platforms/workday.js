...
export const WorkdayPlatform = {
  ...
  /**
   * Fill a multi-select skills field
   * SIMPLE: Type skill → Enter → Wait 1 sec → Click FIRST option → Repeat
   */
  async fillSkillsField(page, selector, skills, label) {
    console.log(`\n  ┌─── DEBUG: fillSkillsField("${label}") ───`);
    console.log(`  │ Selector: ${selector}`);
    ...
    if (!Array.isArray(skills) || skills.length === 0) {
      ...
      console.log(`  └─── END DEBUG ───\n`);
      return { success: false, error: 'No skills' };
    }
    ...
    for (let i = 0; i < skillsToAdd.length; i++) {
      const skill = skillsToAdd[i].trim();
      if (!skill) continue;
      
      console.log(`  │`);
      console.log(`  │ [${i+1}/${skillsToAdd.length}] "${skill}"`);
      
      try {
        // CRITICAL: Close any stale popup from previous skill
        await page.keyboard.press('Escape');
        await sleep(300);
        
        // Verify popup is gone
        const popupGone = await page.evaluate(() => {
          const popups = document.querySelectorAll('[role="listbox"]');
          return popups.length === 0 || Array.from(popups).every(p => {
        ...
        if (!popupGone) {
          console.log(`  │   ⚠️ Popup still visible after Escape, trying again...`);
          await page.keyboard.press('Escape');
          await sleep(300);
        }
        
        // CRITICAL: Click input field to reset Workday's state
        await input.click();
        await sleep(300);
        
        // Verify focus (should be focused from click above)
        ...
        console.log(`  │   Focus: ${focusInfo.focused ? 'YES' : 'NO'} (focused on: ${focusInfo.focusedId})`);
        
        // Clear the input completely and verify
        const beforeClear = await input.evaluate(el => el.value);
        if (beforeClear) {
          console.log(`  │   Input before clear: "${beforeClear}"`);
        }
        
        await input.evaluate(el => el.value = '');
        await sleep(100);
        
        const afterClear = await input.evaluate(el => el.value);
        if (afterClear) {
          console.log(`  │   ⚠️ Input not fully cleared: "${afterClear}"`);
        }
        
        // Type the skill directly into the input element
        console.log(`  │   → Typing "${skill}"...`);
        await input.type(skill, { delay: 50 });
        await sleep(300);
        
        // Verify typing worked
        const typedValue = await input.evaluate(el => el.value);
        console.log(`  │   Field value after typing: "${typedValue}"`);
        
        if (!typedValue || typedValue.trim() === '') {
          console.log(`  │   ❌ Typing failed - field is empty!`);
          await page.keyboard.press('Escape');
          await sleep(200);
          continue;
        }
        ...
        // Wait for popup to appear with CORRECT options for this skill
        await sleep(500);  // Give popup time to start refreshing
        
        // Find and click FIRST option
        console.log(`  │   → Clicking first option...`);
        
        // Try multiple times to find popup with CORRECT options (it might take a moment to refresh)
        let popupFound = null;
        for (let retry = 0; retry < 10; retry++) {
          popupFound = await page.evaluate((inputSel, expectedSkill) => {
            // Get input field position to find closest popup
            const inputEl = document.querySelector(inputSel);
            if (!inputEl) return { found: false };
            ...
            const selectors = [
              '[role="listbox"]',
              '[data-uxi-widget-type="suggestionlist"]',
              '[data-automation-id*="suggestionslist"]',
              '[data-automation-id*="dropdown"]',
              'ul[role="listbox"]',
              'div[role="listbox"]',
              '.WGKV_[role="presentation"]',
              '[class*="suggestion"]',
              '[class*="dropdown"]',
              '[class*="menu"]'
            ];
            ...
            for (const sel of selectors) {
              ...
              for (const popup of elements) {
                ...
                // Check if visible (has size)
                if (rect.width > 0 && rect.height > 0) {
                  // Check if it has options
                  const options = popup.querySelectorAll('[role="option"], li, div[data-automation-id]');
                  if (options.length > 0) {
                    // Calculate distance from input field
                    const distance = Math.abs(rect.top - inputRect.bottom) + Math.abs(rect.left - inputRect.left);
                    
                    // CRITICAL: Only consider popups VERY close to input (<100px)
                    // This filters out stale popups from previous skills
                    if (distance < 100 && distance < closestDistance) {
                      const firstOption = options[0];
                      const optionText = firstOption.textContent?.trim() || '';
                      
                      // CRITICAL: Check if first option matches the expected skill
                      // This prevents accepting stale popups with wrong options
                      const optionLower = optionText.toLowerCase();
                      const skillLower = expectedSkill.toLowerCase();
                      
                      // Check if option contains the skill name (with flexible matching)
                      // e.g., "Python (Programming Language)" contains "python"
                      // e.g., "JavaScript" matches "javascript"
                      // e.g., "C (Programming Language)" matches "c++" (first word)
                      let matches = false;
                      if (optionLower.includes(skillLower)) {
                        matches = true;
                      } else if (skillLower.includes(optionLower.split(' ')[0])) {
                        matches = true;
                      } else {
                        // Check first word of both
                        const optionFirstWord = optionLower.split(' ')[0].split('(')[0];
                        const skillFirstWord = skillLower.split(' ')[0];
                        if (optionFirstWord && skillFirstWord && 
                            (optionFirstWord.includes(skillFirstWord) || skillFirstWord.includes(optionFirstWord))) {
                          matches = true;
                        }
                      }
            ...
            return bestPopup || { found: false };
          }, selector, skill);
          
          if (popupFound.found) {
            console.log(`  │   ✓ Popup found with selector: ${popupFound.selector} (attempt ${retry + 1})`);
            console.log(`  │   ✓ Distance from input: ${popupFound.distance}px, Position: (${popupFound.left}, ${popupFound.top})`);
            console.log(`  │   ✓ Options: ${popupFound.optionCount}, First: "${popupFound.firstOptionText}"`);
            console.log(`  │   ✓ Matches skill "${skill}": YES`);
            break;
          }
          
          // Wait and retry - the NEW popup with CORRECT options might not have appeared yet
          if (retry < 9) {
            if (retry === 0) {
              console.log(`  │   ⏳ Waiting for popup with "${skill}" options...`);
            }
            await sleep(300);
          }
        }
        
        if (!popupFound || !popupFound.found) {
          console.log(`  │   ❌ No popup with "${skill}" options found after 10 attempts`);
          await page.keyboard.press('Escape');
          await sleep(200);
          continue;
        }
        
        // Now click the first option using REAL mouse click (not JavaScript click)
        const clickResult = await page.evaluate((inputSel, expectedSkill) => {
          // Get input field position to find closest popup
          const inputEl = document.querySelector(inputSel);
          if (!inputEl) return { success: false, reason: 'Input not found' };
          
          const inputRect = inputEl.getBoundingClientRect();
          
          // Find popup again (find closest to input)
          const selectors = [
            '[role="listbox"]',
            '[data-uxi-widget-type="suggestionlist"]',
            '[data-automation-id*="suggestionslist"]',
          ...
          for (const selector of selectors) {
            ...
            for (const popup of popups) {
              ...
              if (rect.width > 0 && rect.height > 0) {
                ...
                if (distance < 100 && distance < closestDistance) {
                  // Find first option
                  let firstOption = popup.querySelector('[role="option"]');
                  if (!firstOption) {
                    firstOption = popup.querySelector('li');
                  }
                  if (!firstOption) {
                    firstOption = popup.querySelector('div[data-automation-id]');
                  }
                  
                  if (firstOption) {
                    const optionRect = firstOption.getBoundingClientRect();
                    const optionText = firstOption.textContent?.trim() || 'unknown';
          ...
          return bestOption || { success: false, reason: 'Could not find option in closest popup (within 100px) matching skill "' + expectedSkill + '"' };
        }, selector, skill);
        ...
        console.log(`  │   ✓ Found option at (${Math.round(clickResult.x)}, ${Math.round(clickResult.y)}) [distance: ${clickResult.distance}px]`);
        console.log(`  │   → Clicking with mouse: "${clickResult.skillAdded}"`);
        
        // Use Puppeteer's REAL mouse click
        ...
        const successCheck = await page.evaluate((sel) => {
          const input = document.querySelector(sel);
          if (!input) return { success: false, reason: 'Input not found' };
          
          const container = input.closest('[data-automation-id*="formField"]');
          
          // Method 1: Check if input is clear (skill was consumed)
          const inputClear = !input.value || input.value === '';
          
          // Method 2: Check for skill pills/chips
          const pills = container?.querySelectorAll('[data-automation-id="selectedItem"], [data-automation-id*="pill"], [data-automation-id*="chip"]');
          const hasPills = pills && pills.length > 0;
          
          // Method 3: Check for any new content in container
          const containerHasContent = container && container.textContent.length > 100;
          
          return {
            success: inputClear || hasPills || containerHasContent,
            inputClear,
            pillCount: pills?.length || 0,
            containerLength: container?.textContent?.length || 0
          };
        }, selector);
        
        if (successCheck.success) {
          console.log(`  │   ✅ Added! (pills: ${successCheck.pillCount}, input clear: ${successCheck.inputClear})`);
          addedCount++;
          addedSkills.push(clickResult.skillAdded);
          
          // CRITICAL: Wait for popup to close completely before next skill
          ...
          for (let wait = 0; wait < 10; wait++) {
            const stillVisible = await page.evaluate(() => {
              ...
              for (const popup of popups) {
                const rect = popup.getBoundingClientRect();
                if (rect.width > 0 && rect.height > 0) {
                  return true; // Popup still visible
                }
              }
              return false; // No visible popups
            });
            
            if (!stillVisible) {
              popupClosed = true;
              console.log(`  │   ✓ Popup closed after ${wait * 200}ms`);
              break;
            }
          ...
          if (!popupClosed) {
            console.log(`  │   ⚠️ Popup still visible after 2 seconds, forcing close...`);
            await page.keyboard.press('Escape');
            await sleep(300);
          }
          
        } else {
          console.log(`  │   ⚠️ May have failed (pills: ${successCheck.pillCount}, input clear: ${successCheck.inputClear})`);
        }
    ...
    console.log(`  │`);
    console.log(`  │ ✅ Added ${addedCount}/${skillsToAdd.length} skills`);
    if (addedSkills.length > 0) {
      console.log(`  │ Skills: ${addedSkills.join(', ')}`);
    }
    console.log(`  └─── END DEBUG ───\n`);
  ...
  async fillDropdown(page, selector, values, label, classifier = null) {
    ...
    // CRITICAL: Skip if value is empty/null/undefined (for optional fields like prefix/suffix)
    if (!targetValue || targetValue === '' || targetValue === null || targetValue === undefined) {
      console.log(`  │ ⚠️ Value is empty - skipping this field`);
      console.log(`  └─── END DEBUG ───\n`);
      console.log(`  ⏭️  ${label}: Skipped (empty value)`);
      return { success: true, skipped: true };
    }
    ...
    try {
      ...
      // Collect ALL options by first scrolling to the very bottom
      console.log(`  │ Scrolling to bottom to ensure all options are loaded...`);
      
      // First, scroll to the bottom to load all lazy-loaded options
      for (let scrollAttempt = 0; scrollAttempt < 50; scrollAttempt++) {
        const scrollResult = await page.evaluate((targetTop, targetLeft) => {
          const popups = document.querySelectorAll('[role="listbox"], [data-automation-id="selectBoxPopup"]');
          ...
          for (const popup of popups) {
            ...
            if (rect.height > 30 && Math.abs(rect.top - targetTop) < 50 && Math.abs(rect.left - targetLeft) < 50) {
              ...
              scrollContainer.scrollTop += 200;  // Large increment to get to bottom faster
              ...
              return { scrolled: newScroll > oldScroll, atEnd };
            }
          }
          return { scrolled: false, atEnd: false };
        }, popupTop, popupLeft);
        
        if (!scrollResult.scrolled || scrollResult.atEnd) {
          console.log(`  │ Reached end after ${scrollAttempt + 1} scroll attempts`);
          break;
        }
        await sleep(50);
      }
      ...
      // Now collect ALL options (they're all loaded in the DOM now)
      const allOptionsResult = await page.evaluate((targetTop, targetLeft) => {
        const popups = document.querySelectorAll('[role="listbox"], [data-automation-id="selectBoxPopup"]');
        
        for (const popup of popups) {
          const rect = popup.getBoundingClientRect();
          if (rect.height > 30 && Math.abs(rect.top - targetTop) < 50 && Math.abs(rect.left - targetLeft) < 50) {
            // Query ALL option elements (not just visible ones)
            const options = popup.querySelectorAll('[data-automation-id="promptOption"], [role="option"]');
            const optionTexts = [];
            
            for (const opt of options) {
              const text = opt.textContent?.trim() || '';
              // Don't filter by visibility - collect ALL options
              if (text.length >= 2 && text.length < 200 && text.toLowerCase() !== 'select one') {
                optionTexts.push(text);
              }
            }
            
            // Remove duplicates and return
            return { options: Array.from(new Set(optionTexts)), found: true };
          }
        }
        return { options: [], found: false };
      }, popupTop, popupLeft);
      
      if (!allOptionsResult.found) {
        console.log(`  │ ❌ Could not find popup to collect options`);
        await page.keyboard.press('Escape');
        console.log(`  └─── END DEBUG ───\n`);
        return { success: false, error: 'Popup lost' };
      }
      
      const sortedOptions = allOptionsResult.options.sort();
      console.log(`  │   Collected ${sortedOptions.length} unique options`);
      
      // Sanity check
      if (targetPopup.optionCount >= 5 && sortedOptions.length < targetPopup.optionCount - 1) {
        console.log(`  │   ⚠️ WARNING: Expected ${targetPopup.optionCount} options but only got ${sortedOptions.length}`);
        console.log(`  │   Collected: ${sortedOptions.join(' | ')}`);
      }
  ...
  async findBestOptionWithAI(targetValue, fieldLabel, options, classifier) {
    ...
    // CRITICAL: For veteran status, ALWAYS use semantic matching first
    // Keyword matching fails because it doesn't understand negation:
    // "I am NOT a veteran" vs "I AM a veteran, but not protected" both have "am", "not", "veteran"
    const isVeteranStatus = (fieldLabel || '').toLowerCase().includes('veteran');
    const isDisabilityStatus = (fieldLabel || '').toLowerCase().includes('disability');
    const isSensitiveField = isVeteranStatus || isDisabilityStatus;
    
    // STEP 1: Try semantic similarity FIRST for sensitive fields
    if (isSensitiveField) {
      console.log(`  │     ⚠️ Sensitive field detected - using semantic matching with Haiku verification`);
      try {
        ...
        // Score ALL options for sensitive fields
        ...
        for (const scored of scoredOptions.slice(0, 5)) {
          console.log(`  │       ${(scored.score * 100).toFixed(1)}% - "${scored.option.substring(0, 60)}"`);
        }
        
        // HAIKU VERIFICATION: BGE can be fooled by word overlap
        // Ask Haiku to verify the top matches make sense
        if (scoredOptions.length > 0 && scoredOptions[0].score > 0.3) {
          console.log(`  │     🔍 Top semantic match: "${scoredOptions[0].option.substring(0, 60)}" (${(scoredOptions[0].score * 100).toFixed(1)}%)`);
          console.log(`  │     ⚙️ Asking Haiku to verify matches...`);
          
          // Try up to top 10 options (or ALL if < 10)
          // For veteran/disability status, there are typically only 4-6 options, so we'll try them all
          const optionsToTry = Math.min(scoredOptions.length, 10);
          
          for (let i = 0; i < optionsToTry; i++) {
            const match = scoredOptions[i];
            console.log(`  │     🔍 Trying option ${i + 1}/${optionsToTry}: "${match.option.substring(0, 60)}" (${(match.score * 100).toFixed(1)}%)`);
            
            // Check if we have Anthropic API access (from verified classifier if available)
            const haikuConfirmed = await this.verifyMatchWithHaiku(targetValue, match.option, fieldLabel);
            
            if (haikuConfirmed) {
              console.log(`  │     ✅ Haiku confirmed option ${i + 1}: "${match.option.substring(0, 60)}"`);
              return {
                bestMatch: match.option,
                confidence: match.score
              };
            } else {
              console.log(`  │     ⚠️ Haiku rejected option ${i + 1}`);
            }
          }
          
          // All top options rejected, fall through
          console.log(`  │     ⚠️ Haiku rejected all top ${optionsToTry} options - falling back to keyword matching`);
        }
      } catch (e) {
        console.log(`  │     Semantic similarity error: ${e.message}, falling back to keywords`);
      }
    }
    
    // STEP 2: Try comprehensive keyword matching (for non-sensitive fields or if semantic failed)
    const keywordResult = this.findBestOptionByKeywords(targetValue, fieldLabel, options);
    if (keywordResult.bestMatch && keywordResult.confidence >= 0.70) {  // 70% threshold - good keyword matches should be trusted
      console.log(`  │     ✓ Keyword match found: "${keywordResult.bestMatch}" (${(keywordResult.confidence * 100).toFixed(0)}%)`);
      return keywordResult;
    }
    
    // STEP 3: Try semantic similarity for non-sensitive fields
    if (!isSensitiveField) {
      try {
        const SemanticSimilarityModule = await import('../semantic-similarity.js');
        const SemanticSimilarityClass = SemanticSimilarityModule.default;
        ...
        // Limit to first 100 options for performance
    ...
    // STEP 4: Fall back to keyword matching even with lower confidence
  ...
  /**
   * HAIKU VERIFICATION FOR DROPDOWN MATCHING
   * Verifies that a semantic match actually makes sense semantically
   * Similar to Tier 2 verification in Trust Cascade
   */
  async verifyMatchWithHaiku(profileAnswer, dropdownOption, fieldLabel) {
    try {
      // Try to get Anthropic API key from environment
      const apiKey = process.env.ANTHROPIC_API_KEY;
      if (!apiKey) {
        console.log(`  │     ⚠️ No Anthropic API key found - skipping Haiku verification`);
        return true; // Default to trusting semantic match if no API key
      }

      const Anthropic = (await import('@anthropic-ai/sdk')).default;
      const client = new Anthropic({ apiKey });

      const prompt = `You are verifying a dropdown selection match.

FIELD: "${fieldLabel}"
USER'S ANSWER: "${profileAnswer}"
PROPOSED DROPDOWN OPTION: "${dropdownOption}"

Does this dropdown option accurately represent the user's answer? Consider:
- Semantic meaning (not just word overlap)
- Negation ("not a veteran" vs "is a veteran")
- Opposite meanings that share similar words
...
Respond with ONLY "YES" or "NO", nothing else.`;
      ...
      console.log(`  │     ⚠️ Haiku verification error: ${error.message}`);
      return true; // Default to trusting semantic match on error
    }
  }
};

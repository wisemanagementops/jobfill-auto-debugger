// ============================================
// AI-POWERED FORM FILLER (v5 - Fixed Cache Design)
// ============================================
// THREE-STAGE CLASSIFICATION:
//   Stage 1: Zero-Shot (DeBERTa) - 45% threshold
//   Stage 2: Semantic Similarity (BGE) - 60% threshold
//   Stage 3: VERIFIED CLASSIFIER v5 - Pattern Cache + Claude
//
// *** CRITICAL FIX IN v5 ***
// Cache stores FIELD TYPE only, answers come from PROFILE!
// This ensures different users get their own correct answers.
//
// Example:
//   Cache: "First Name" → first_name
//   Profile A: first_name → "Bhanu"
//   Profile B: first_name → "John"
// ============================================
...
import VerifiedFieldClassifier from './verified-field-classifier-v8.js';  // v8: Trust Cascade!
...
const DATE_CONTEXTS = {
  ...
  EDUCATION_START: 'education_start_date',
...
export class AIFormFiller {
  constructor(page, profile) {
    ...
    // NEW: Initialize the SMART VerifiedFieldClassifier with profile
    // Claude will verify types AND determine answers using the full profile!
    this.verifiedClassifier = new VerifiedFieldClassifier({
      stage1Classifier: this.stage1Classifier,
      stage2Classifier: this.stage2Classifier,
      profile: this.profile,  // Pass profile so Claude can make smart inferences!
    });
  }
  ...
  // ============================================
  // ============================================
  // COMPANY-SPECIFIC EMPLOYMENT DETECTION (GENERAL)
  // Detects questions like "Have you ever been employed by [Company]?"
  // ============================================
  isCompanySpecificEmploymentQuestion(sectionLower, contextLower) {
    const text = sectionLower + ' ' + contextLower;
    
    // Pattern: "employed by [company]" / "partner at [company]" / "worked at [company]"
    // Must mention BOTH an employment verb AND a specific company context
    // Exclude generic "previously employed" / "employed at this company" questions
    const hasEmploymentVerb = (
      text.includes('employed by') || text.includes('worked at') || text.includes('worked for') ||
      text.includes('partner of') || text.includes('partner at') || text.includes('partner and') ||
      text.includes('been employed by') || text.includes('ever been employed') ||
      text.includes('were you a partner') || text.includes('affiliated with') ||
      text.includes('contractor for') || text.includes('consultant for') ||
      text.includes('engaged by') || text.includes('internship at') || text.includes('interned at')
    );
    
    if (!hasEmploymentVerb) return false;
    
    // Must mention a SPECIFIC named company (not just "this company" / "our company")
    // Look for proper nouns: capitalized words, abbreviations (LLC, LLP, Inc, Corp)
    // or well-known company indicators
    const hasCompanyIndicator = (
      text.includes('llp') || text.includes('llc') || text.includes('inc') || 
      text.includes('corp') || text.includes('ltd') || text.includes('co.') ||
      text.includes('group') || text.includes('partners') || text.includes('& co') ||
      // Well-known companies that appear in compliance questions
      text.includes('kpmg') || text.includes('deloitte') || text.includes('pwc') || 
      text.includes('ernst') || text.includes('accenture') || text.includes('mckinsey') ||
      text.includes('goldman') || text.includes('morgan') || text.includes('jpmorgan') ||
      text.includes('booz') || text.includes('bain') || text.includes('bcg') ||
      // General pattern: "employed by X and/or its members/affiliates/subsidiaries"
      text.includes('affiliates') || text.includes('subsidiaries') || text.includes('members and')
    );
    
    // Exclude generic questions about THIS company (the one being applied to)
    const isGenericPreviouslyEmployed = (
      (text.includes('this company') || text.includes('our company') || text.includes('our organization')) &&
      !hasCompanyIndicator
    );
    
    return hasCompanyIndicator && !isGenericPreviouslyEmployed;
  }
  
  extractCompanyFromQuestion(text) {
    // Try to extract the company name from "employed by [Company]" patterns
    const patterns = [
      /(?:employed by|worked (?:at|for)|partner (?:of|at)|affiliated with|contractor for)\s+([A-Z][A-Za-z\s&.,]+?)(?:\s+(?:and\/or|or|in|within|during|worldwide|globally|in the|for the))/i,
      /(?:employed by|worked (?:at|for)|partner (?:of|at))\s+([A-Z][A-Za-z\s&.,]+?)(?:\s*(?:LLP|LLC|Inc|Corp|Ltd|Co\.))/i,
      /(?:employed by|partner (?:of|at))\s+([A-Z][A-Za-z\s&.,]+?)(?:\?|\*)/i,
    ];
    ...
    const knownCompanies = [
      'KPMG', 'Deloitte', 'PwC', 'PricewaterhouseCoopers', 'Ernst & Young', 'EY',
      'Goldman Sachs', 'Morgan Stanley', 'JPMorgan', 'JP Morgan', 'Citi', 'Citigroup',
      'McKinsey', 'BCG', 'Boston Consulting', 'Bain', 'Accenture', 'Booz Allen',
      'Amazon', 'Google', 'Meta', 'Microsoft', 'Apple', 'Tesla', 'Netflix',
    ];
  ...
  detectDateContext(field, sectionText, allFields = []) {
    ...
    if ((labelLower.includes('start date') || labelLower.includes('from date') ||
      ...
      if (this.isEducationContext(allText, sectionLower, allFields, field)) {
        console.log(`      🗓️ Date Context: EDUCATION START (from/start in education)`);
        return DATE_CONTEXTS.EDUCATION_START;
      }
    ...
    if (labelLower.includes('end date') || labelLower.includes('to date') ||
        ...
        labelLower.includes('actual or expected') || labelLower.includes('to (actual')) {
      // Check if education context — "To" in education = graduation year
      if (this.isEducationContext(allText, sectionLower, allFields, field)) {
        console.log(`      🗓️ Date Context: GRADUATION DATE (to/end in education)`);
        return DATE_CONTEXTS.GRADUATION_DATE;
      }
  ...
  getDateValue(dateContext, field, profile) {
    ...
    switch (dateContext) {
      ...
      case DATE_CONTEXTS.EDUCATION_START:
        // Try explicit start year, fall back to inference from degree type
        if (p.education?.startYear) {
          if (datePart.includes('year')) return String(p.education.startYear);
          if (datePart.includes('month')) return '8'; // August (common semester start)
          if (datePart.includes('day')) return '15';
          return `8/15/${p.education.startYear}`;
        }
        // Infer from graduation year and degree type
        if (p.education?.graduationYear) {
          const gradYear = parseInt(p.education.graduationYear);
          const degreeLower = (p.education?.degree || '').toLowerCase();
          let duration = 4; // Default: Bachelor's
          if (degreeLower.includes('master') || degreeLower.includes('mba') || degreeLower.includes('m.s') || degreeLower.includes('m.a')) duration = 2;
          else if (degreeLower.includes('phd') || degreeLower.includes('doctor')) duration = 5;
          else if (degreeLower.includes('associate')) duration = 2;
          const startYear = gradYear - duration;
          console.log(`         🧮 Inferred education start: ${startYear} (${p.education.degree} = ${duration} years before ${gradYear})`);
          if (datePart.includes('year')) return String(startYear);
          if (datePart.includes('month')) return '8';
          if (datePart.includes('day')) return '15';
          return `8/15/${startYear}`;
        }
        console.log(`         ⚠️ Education start date not in profile - skipping`);
        return null;
  ...
  async fillAllFields() {
    ...
    // v5: Set application context for cache (extracts ATS platform and company)
    const currentUrl = this.page.url();
    this.verifiedClassifier.setApplicationContext(currentUrl);
  ...
  async discoverFields() {
    const rawFields = await this.page.evaluate(() => {
      ...
      const SKIP_PATTERNS = [
        ...
        /^logo/i,
        /^icon/i,
        /honeypot/i,     // Anti-spam trap fields - NEVER fill these!
        /honey-pot/i,    // Variant spelling
        /honey_pot/i     // Another variant
      ];
      ...
      // ============================================================================
      // ULTRA-CONSERVATIVE HONEYPOT DETECTION
      // Only block fields that are OBVIOUSLY honeypots - avoid ALL false positives!
      // Better to fill a honeypot than miss required fields!
      // ============================================================================
      function isHoneypot(el) {
        // ONLY check obvious honeypot names
        ...
        // ONLY block if name explicitly contains honeypot-related terms
        if (allText.includes('honeypot') || 
            allText.includes('honey-pot') || 
            allText.includes('honey_pot') ||
            allText.includes('bot-field') ||
            allText.includes('spam-trap')) {
          return true;
        }
        
        // Check only the most obvious CSS hiding
        const style = window.getComputedStyle(el);
        
        // ONLY display:none (most common and clear)
        if (style.display === 'none') {
          return true;
        }
        
        // That's it! No other checks.
        // We're accepting the risk of occasionally filling honeypots
        // to ensure we NEVER miss legitimate fields
      ...
      function getSection(el) {
        ...
        let foundQuestionInContainer = false;
        ...
        if (container) {
          ...
          if (questionEl) {
            ...
            if (qText && qText.length > 10 && !qText.toLowerCase().includes('select one')) {
              parts.push(qText);
              foundQuestionInContainer = true;
            }
          }
          
          // ONLY look at previous siblings if we did NOT find question text inside the container
          // And limit to just 1 sibling — going further picks up UNRELATED questions
          // (e.g., involuntary discharge dropdown picking up sponsorship question text)
          if (!foundQuestionInContainer) {
        ...
        // Only look at element's parent sibling if we still have NO question text
        if (!foundQuestionInContainer && parts.length <= 1) {
      ...
      document.querySelectorAll('input[type="text"], input[type="email"], input[type="tel"], input:not([type])').forEach(input => {
        ...
        if (isHoneypot(input)) return; // Skip honeypot fields
      ...
      document.querySelectorAll('textarea').forEach(textarea => {
        ...
        if (isHoneypot(textarea)) return; // Skip honeypot fields
      ...
      document.querySelectorAll('button[aria-haspopup="listbox"]').forEach(btn => {
        ...
        if (isHoneypot(btn)) return; // Skip honeypot fields
      ...
      document.querySelectorAll('input[type="radio"]').forEach(radio => {
        ...
        if (isHoneypot(radio)) return; // Skip honeypot fields
      ...
      document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        ...
        if (isHoneypot(cb)) return; // Skip honeypot fields
  ...
  async classifyAllFields(fields) {
    ...
    console.log('\n\n🔄 Using VERIFIED Classification v3 (All Signals + Claude Verifies + SMART ANSWERS)');
    console.log('══════════════════════════════════════════════════════════════════════');
    console.log('   Step 1: Collect ALL signals (Field ID + Options + Label + Local AI)');
    console.log('   Step 2: Aggregate into proposed type + proposed answer');
    console.log('   Step 3: Claude verifies type AND determines answer from FULL PROFILE');
    console.log('   Step 4: Conflict resolution if signals disagree with Claude');
    console.log('   Step 5: Fill with Claude\'s answer - ZERO manual overrides!');
    console.log('══════════════════════════════════════════════════════════════════════\n');
    
    // Give classifier awareness of ALL fields on this page (for batch context)
    this.verifiedClassifier.setPageFields(fields);
    
    for (let i = 0; i < fields.length; i++) {
      const field = fields[i];
      const fieldNum = i + 1;
      
      process.stdout.write(`   [${fieldNum}/${fields.length}] 🧠 "${field.label || field.id}"...`);
      console.log('');
      
      // Use the SMART verified classifier - returns both type AND answer!
      const result = await this.verifiedClassifier.classifyField(field);
      
      // Build classification object with Claude's answer
      const classification = {
        label: result.field_type,
        confidence: result.confidence,
        source: result.source,
        answer: result.answer,  // v7: answer from AI consensus or Claude fallback
      };
      
      // Show result
      if (result.answer) {
        console.log(`      → ${result.field_type} (${(result.confidence * 100).toFixed(1)}%) [${result.source}] ✓`);
      ...
      classifications.push({
        field,
        classification,
        context: field.section || field.label,
        claudeAnswer: result.answer,  // Store Claude's answer separately
      });
    }
    
    // Print stats
    this.verifiedClassifier.printStats();
  ...
  async fillClassifiedFields(classifications) {
    ...
    // Track section overrides that should only fire ONCE per page
    // (e.g., there's only ONE sponsorship dropdown per application)
    const usedSingletonOverrides = new Set();
    
    for (const { field, classification, context, claudeAnswer } of classifications) {
      ...
      // SECTION-BASED OVERRIDES (only for GENERIC classifications)
      // Don't override if AI already gave us a SPECIFIC type
      const specificTypes = [
        'scp_referral', 'kpmg_employment', 'big_four_employment', 
        'military_service', 'sponsorship_details', 'company_specific_employment',
        'involuntary_termination', 'employee_referral'
      ];
      const classificationLabelLower = (classification.label || '').toLowerCase();
      const isGenericClassification = !specificTypes.includes(classificationLabelLower);
      
      // Work Authorization questions
      if (isGenericClassification && (
          sectionLower.includes('authorized to work') || contextLower.includes('authorized to work') ||
      ...
      // ============================================
      // EMPLOYEE REFERRAL — "Were you referred to [Company]?"
      // Different from "How did you hear about us?" (referral_source)
      // This asks about personal referrals by employees/clients → "No"
      // ============================================
      else if ((sectionLower.includes('were you referred') || contextLower.includes('were you referred') ||
                sectionLower.includes('did someone refer') || contextLower.includes('did someone refer') ||
                sectionLower.includes('referred by a') || contextLower.includes('referred by a') ||
                sectionLower.includes('referred for employment') || contextLower.includes('referred for employment')) &&
               !sectionLower.includes('how did you') && !contextLower.includes('how did you')) {
        effectiveLabel = 'employee_referral';
        console.log(`      📋 Section Override: employee referral (question text match)`);
      }
      // ============================================
      // INVOLUNTARY TERMINATION / DISCHARGE / FORCED RESIGNATION
      // Always "No" - MUST come BEFORE sponsorship override!
      // (On Workday, the section text often contains both sponsorship AND discharge text)
      // ============================================
      else if (sectionLower.includes('involuntarily discharged') || contextLower.includes('involuntarily discharged') ||
               sectionLower.includes('involuntary discharge') || contextLower.includes('involuntary discharge') ||
               sectionLower.includes('asked to resign') || contextLower.includes('asked to resign') ||
               sectionLower.includes('forced to resign') || contextLower.includes('forced to resign') ||
               sectionLower.includes('terminated from') || contextLower.includes('terminated from') ||
               sectionLower.includes('involuntarily terminated') || contextLower.includes('involuntarily terminated') ||
               sectionLower.includes('dismissed from') || contextLower.includes('dismissed from') ||
               (sectionLower.includes('let go') && (sectionLower.includes('position') || sectionLower.includes('employment'))) ||
               (contextLower.includes('let go') && (contextLower.includes('position') || contextLower.includes('employment')))) {
        effectiveLabel = 'involuntary_termination';
        console.log(`      📋 Section Override: involuntary termination / forced resignation (question text match)`);
      }
      // Visa Sponsorship DETAILS (textarea asking for explanation)
      // Must come BEFORE the sponsorship_yes_no override!
      else if ((field.type === 'textarea' || fieldLabelLower.includes('detail') || fieldLabelLower.includes('explain') ||
                fieldLabelLower.includes('additional') || fieldLabelLower.includes('provide')) &&
               (sectionLower.includes('sponsorship') || contextLower.includes('sponsorship') ||
                fieldLabelLower.includes('sponsorship'))) {
        effectiveLabel = 'sponsorship_details';
        console.log(`      📋 Section Override: sponsorship DETAILS textarea (not yes/no)`);
      }
      // Visa Sponsorship questions (but NOT sponsorship_details!)
      // SINGLETON: Only ONE sponsorship dropdown per application
      else if (isGenericClassification && classificationLabelLower !== 'sponsorship_details' && 
               !usedSingletonOverrides.has('sponsorship_yes_no') && (
               sectionLower.includes('require sponsorship') || contextLower.includes('require sponsorship') ||
        ...
        usedSingletonOverrides.add('sponsorship_yes_no');
        console.log(`      📋 Section Override: sponsorship (question text match)`);
      }
      // DUPLICATE SPONSORSHIP GUARD: If sponsorship override already fired but
      // section text still contains sponsorship (bleed from neighboring question),
      // this field is NOT sponsorship — it's a misidentified compliance question.
      // Default to disciplinary_action (safe "No") since compliance questions near
      // sponsorship are always negative-answer questions (involuntary discharge, etc.)
      else if (isGenericClassification && usedSingletonOverrides.has('sponsorship_yes_no') && (
               sectionLower.includes('require sponsorship') || contextLower.includes('require sponsorship') ||
               sectionLower.includes('need sponsorship') || contextLower.includes('need sponsorship') ||
               sectionLower.includes('visa sponsorship') || contextLower.includes('visa sponsorship') ||
               (sectionLower.includes('sponsorship') && sectionLower.includes('visa')))) {
        effectiveLabel = 'disciplinary_action';
        console.log(`      ⚠️ DUPLICATE SPONSORSHIP BLOCKED: Already have a sponsorship dropdown on this page`);
        console.log(`      🔄 Defaulting to disciplinary_action (safe "No") — likely a misidentified compliance question`);
      }
      // Relative at company questions (but NOT scp_referral!)
      else if (isGenericClassification && classificationLabelLower !== 'scp_referral' && (
               sectionLower.includes('relative') || contextLower.includes('relative') ||
               ...
               !sectionLower.includes('senior commercial') && !contextLower.includes('senior commercial')) {
      ...
      else if (isGenericClassification && (
               sectionLower.includes('non-compete') || contextLower.includes('non-compete') ||
      ...
      // Background check consent
               ...
               sectionLower.includes('background check') || contextLower.includes('background check') ||
      ...
      // Drug test consent
               ...
               sectionLower.includes('drug test') || contextLower.includes('drug test') ||
      ...
      // Previously employed questions (but NOT kpmg_employment or big_four_employment!)
      else if (isGenericClassification && classificationLabelLower !== 'kpmg_employment' && classificationLabelLower !== 'big_four_employment' && (
               sectionLower.includes('previously employed') || contextLower.includes('previously employed') ||
      ...
      // Currently employed questions
               ...
               sectionLower.includes('currently employed') || contextLower.includes('currently employed') ||
      ...
      // Contact current employer questions
               ...
               sectionLower.includes('contact your current employer') || contextLower.includes('contact your current employer') ||
      ...
      // ============================================
      // MILITARY SERVICE / ARMED FORCES (must come BEFORE over_18!)
      // Opus often confuses "Armed Forces" with age_verification
      // ============================================
      else if (sectionLower.includes('armed forces') || contextLower.includes('armed forces') ||
               sectionLower.includes('military service') || contextLower.includes('military service') ||
               sectionLower.includes('served in the armed') || contextLower.includes('served in the armed') ||
               sectionLower.includes('national guard') || contextLower.includes('national guard') ||
               (sectionLower.includes('serving') && sectionLower.includes('military')) ||
               (contextLower.includes('serving') && contextLower.includes('military'))) {
        effectiveLabel = 'military_service';
        console.log(`      📋 Section Override: military service / Armed Forces (question text match)`);
      }
      // ============================================
      // COMPANY-SPECIFIC EMPLOYMENT (GENERAL SEMANTIC DETECTION)
      // Catches: "Were you employed by [Company]?", "Have you worked at [Company]?",
      //          "Were you a partner at [Company]?", etc.
      // Strategy: Extract company name → check workHistory → answer Yes/No
      // ============================================
      else if (this.isCompanySpecificEmploymentQuestion(sectionLower, contextLower)) {
        effectiveLabel = 'company_specific_employment';
        // Store the question context so the value mapper can extract company names
        field._employmentQuestionContext = sectionLower || contextLower;
        const detectedCompany = this.extractCompanyFromQuestion(field._employmentQuestionContext);
        console.log(`      📋 Section Override: company-specific employment (detected: "${detectedCompany || 'unknown'}")`);
      }
      // Over 18 / legal age questions (EXCLUDES armed forces/military!)
      else if (isGenericClassification && (
               sectionLower.includes('over 18') || contextLower.includes('over 18') ||
               ...
               !sectionLower.includes('armed forces') && !contextLower.includes('armed forces') &&
               !sectionLower.includes('military') && !contextLower.includes('military')) {
        effectiveLabel = 'over_18';
        console.log(`      📋 Section Override: over 18 (question text match)`);
      }
      // Criminal history questions
      else if (isGenericClassification && (
               sectionLower.includes('convicted') || contextLower.includes('convicted') ||
      ...
      // Willing to relocate questions  
               ...
               sectionLower.includes('willing to relocate') || contextLower.includes('willing to relocate') ||
      ...
      // Willing to travel questions
               ...
               sectionLower.includes('willing to travel') || contextLower.includes('willing to travel') ||
      ...
      // Citizenship questions - "Country of Citizenship"
               ...
               sectionLower.includes('country of citizenship') || contextLower.includes('country of citizenship') ||
      ...
      // Restricted countries citizenship question (Group D countries, export control)
               ...
               sectionLower.includes('citizen of any of the following countries') || 
      ...
      // ============================================
      // GPA / Academic Result fields
      // ============================================
      else if (fieldLabelLower.includes('gpa') || fieldLabelLower.includes('overall result') ||
               fieldLabelLower.includes('grade point') || fieldLabelLower.includes('cumulative gpa') ||
               fieldIdLower.includes('gpa') || fieldIdLower.includes('overallresult')) {
        effectiveLabel = 'gpa';
        console.log(`      📋 Override: GPA (field label match)`);
      }
      // ============================================
      ...
      // Also exclude "extension" fields and "phone type" fields
      else if ((fieldLabelLower.includes('phone number') && !fieldLabelLower.includes('code') && !fieldLabelLower.includes('country') && !fieldLabelLower.includes('extension') && !fieldLabelLower.includes('type')) ||
               (fieldIdLower.includes('phonenumber') && !fieldIdLower.includes('countryphone') && !fieldIdLower.includes('phonecode') && !fieldIdLower.includes('extension') && !fieldIdLower.includes('phonetype') && !fieldIdLower.includes('type'))) {
      ...
      // Detect date fields by label (month, day, year, date) or AI classification
        ...
        } else if (dateContext === 'education_start_date') {
          effectiveLabel = 'education_start_date';
      ...
      // Priority 1: Use Claude's answer if available AND label wasn't overridden
      // Priority 2: If label was overridden, recalculate with new label
      // Priority 3: Fall back to value mapper
      let value = null;
      
      // Check if label was overridden (effectiveLabel != original classification)
      const labelWasOverridden = (effectiveLabel !== classification.label);
      
      // SPECIAL CASE: CPT/OPT detection
      // If classified as work_authorization but question is about CPT/OPT,
      // override the answer to use the correct profile field
      let adjustedClaudeAnswer = claudeAnswer;
      if (classification.label === 'work_authorization' && claudeAnswer !== null) {
        // Check if question mentions CPT or OPT
        const questionText = ((field?.label || '') + ' ' + (field?.section || '')).toLowerCase();
        
        if (questionText.includes('cpt') || 
            questionText.includes('opt') ||
            questionText.includes('curricular practical training') ||
            questionText.includes('optional practical training')) {
          // This is about CPT/OPT, not general work auth
          adjustedClaudeAnswer = this.profile.workAuth?.involvesCPT_OPT ? 'Yes' : 'No';
          console.log(`      🔧 CPT/OPT detected - overriding answer to: "${adjustedClaudeAnswer}"`);
        }
      }
      
      // Check if Claude provided an answer (from the verified classifier)
      if (adjustedClaudeAnswer !== null && adjustedClaudeAnswer !== undefined && !labelWasOverridden) {
        // Use Claude's SMART answer - but ONLY if label wasn't overridden!
        value = adjustedClaudeAnswer;
        console.log(`      🤖 Using Claude's answer: "${value}"`);
      } else {
        // Fall back to dumb value mapper (or if label was overridden)
        value = this.getValueForClassification(effectiveLabel, field);
        if (value) {
          if (labelWasOverridden) {
            console.log(`      🔄 Label overridden: "${classification.label}" → "${effectiveLabel}"`);
            console.log(`      📦 Recalculated answer: "${value}"`);
          } else {
            console.log(`      📦 Using value mapper: "${value}"`);
          }
        }
      }
  ...
  getValueForClassification(label, field) {
    ...
    if (labelLower === 'prefix') return p.personal?.prefix || '';  // Leave blank if not set
    if (labelLower === 'suffix') return p.personal?.suffix || '';  // Leave blank if not set
    ...
    if (labelLower === 'phone_type') return 'Mobile';  // Default to mobile
    ...
    // SPECIAL CASE: CPT/OPT questions (specific type of work authorization for F-1 students)
    // Check if this is asking about CPT/OPT specifically, not general work authorization
    if (labelLower === 'work_authorization') {
      const questionText = (field?.label || '') + ' ' + (field?.section || '');
      const questionLower = questionText.toLowerCase();
      
      // If question mentions CPT or OPT, use the specific CPT/OPT field
      if (questionLower.includes('cpt') || 
          questionLower.includes('opt') ||
          questionLower.includes('curricular practical training') ||
          questionLower.includes('optional practical training')) {
        return p.workAuth?.involvesCPT_OPT ? 'Yes' : 'No';
      }
      
      // Otherwise, use general work authorization
    ...
    // Employee referral — "Were you referred to [Company] by an employee?"
    // Different from referral_source (how did you hear about us)
    // If profile has a referrer name, answer "Yes", otherwise "No"
    if (labelLower === 'employee_referral') {
      if (p.referral?.referrerName) return 'Yes';
      return 'No';
    }
    ...
    if (labelLower === 'gpa') {
      const gpa = p.education?.gpa;
      if (gpa) return String(gpa);
      console.log(`      ⚠️ GPA not in profile - skipping`);
      return null;
    }
    ...
    // ALWAYS return "Yes" to avoid ATS auto-rejection
    // If user is applying, they likely believe they meet the requirements
    if (labelLower === 'meets_educational_requirements') {
      return 'Yes';  // Optimistic - let human recruiters decide
    }
    if (labelLower === 'meets_job_requirements') {
      return 'Yes';  // Optimistic - let human recruiters decide
    }
    
    // ========== COMPLIANCE/BACKGROUND ==========
    // Healthcare program exclusion (Medicare/Medicaid)
    if (labelLower === 'healthcare_exclusion') {
      return p.additional?.excludedFromHealthcarePrograms ? 'Yes' : 'No';
    }
    // Disciplinary action on professional license/certification
    if (labelLower === 'disciplinary_action') {
      return p.additional?.hasDisciplinaryAction ? 'Yes' : 'No';
    }
    
    // Involuntary termination / discharge / forced resignation - always "No"
    if (labelLower === 'involuntary_termination') {
      return 'No';
    }
    
    // Military service (Armed Forces) - INFER from veteran status
    // Logic: If not a veteran, never served in military
    if (labelLower === 'military_service') {
      // Check EEO section for veteran status
      const veteranStatus = p.eeo?.veteran_status || p.eeo?.veteranStatus;
      if (typeof veteranStatus === 'string') {
        const vs = veteranStatus.toLowerCase();
        // Any "not a veteran" / "not a protected veteran" / "I am not" → No
        if (vs.includes('not a veteran') || vs.includes('not a protected') || 
            vs.includes('i am not') || vs.includes('never served') ||
            vs === 'no' || vs === 'n/a') {
          return 'No';
        }
        // Any explicit yes / veteran confirmation → Yes
        if (vs.includes('i am a veteran') || vs.includes('i am a protected') ||
            vs === 'yes') {
          return 'Yes';
        }
      }
      if (veteranStatus === false || veteranStatus === 'No') {
        return 'No';
      }
      if (veteranStatus === true || veteranStatus === 'Yes') {
        return 'Yes';
      }
      // Default: No (most people haven't served)
      return 'No';
    }
    
    // SCP (Senior Commercial Person) referral - INFER from referral data
    // Logic: If no referral information in profile, answer No
    if (labelLower === 'scp_referral') {
      // Profile doesn't track referrals → not a referral
      return 'No';
    }
    
    // ============================================
    // COMPANY-SPECIFIC EMPLOYMENT (GENERAL HANDLER)
    // Handles: "Have you worked at [Company]?", "Were you a partner at [Company]?"
    // Strategy: Extract company name from question → check workHistory → Yes/No
    // ============================================
    if (labelLower === 'company_specific_employment' || labelLower === 'kpmg_employment' || labelLower === 'big_four_employment') {
      const workHistory = p.workHistory || p.experience || [];
      const questionContext = (field?._employmentQuestionContext || field?.section || '').toLowerCase();
      const detectedCompany = this.extractCompanyFromQuestion(questionContext);
      
      if (detectedCompany) {
        const companyLower = detectedCompany.toLowerCase();
        const workedThere = workHistory.some(job => {
          const jobCompany = (job.company || job.employer || '').toLowerCase();
          return jobCompany.includes(companyLower) || companyLower.includes(jobCompany);
        });
        console.log(`      🏢 Company employment check: "${detectedCompany}" → workHistory has ${workHistory.length} entries → ${workedThere ? 'MATCH' : 'NO MATCH'}`);
        return workedThere ? 'Yes' : 'No';
      }
      
      // Fallback: if no company detected, check for known companies in text
      const knownCompanies = ['kpmg', 'deloitte', 'pwc', 'pricewaterhousecoopers', 'ernst & young', 'ey',
                              'goldman sachs', 'morgan stanley', 'jpmorgan', 'mckinsey', 'bcg', 'bain', 'accenture'];
      for (const company of knownCompanies) {
        if (questionContext.includes(company)) {
          const workedThere = workHistory.some(job => {
            const jobCompany = (job.company || job.employer || '').toLowerCase();
            return jobCompany.includes(company);
          });
          console.log(`      🏢 Company employment check (fallback): "${company}" → ${workedThere ? 'MATCH' : 'NO MATCH'}`);
          return workedThere ? 'Yes' : 'No';
        }
      }
      
      // Last resort: empty workHistory = never worked anywhere specific
      console.log(`      🏢 Company employment check: no company detected, workHistory=${workHistory.length} → defaulting to No`);
      return 'No';
    }
    
    // Sponsorship details (text explanation) - GENERATE from visa info
    // Logic: Construct explanation based on visaStatus and sponsorship needs
    if (labelLower === 'sponsorship_details') {
      if (p.workAuth?.requiresSponsorship) {
        const visaStatus = p.workAuth?.visaStatus || 'H1B';
        return `I am currently on ${visaStatus} status and will require visa sponsorship for continued employment in the United States.`;
      }
      return null; // No sponsorship needed
    }
    ...
    // CRITICAL: Check line 2 FIRST - otherwise "address line 2" matches "address line" in line 1 condition!
    if (labelLower === 'address line 2' || labelLower.includes('apartment') || labelLower.includes('suite') || labelLower.includes('unit')) {
      return p.address?.line2 || '';  // Return empty string, not null
    }
    // Line 1 check - explicitly exclude "line 2"
    if (labelLower === 'address line 1' || 
        (labelLower.includes('address line') && !labelLower.includes('line 2')) ||
        labelLower.includes('street address') || 
        labelLower.includes('mailing address')) {
    ...
    // County - look up from zip code using external API/database
    if (labelLower === 'county') {
      // First check if county is in profile
      if (p.address?.county) {
        return p.address.county;
      }
      // For now, return placeholder - will implement async lookup
      // TODO: Implement Haiku lookup for county based on zip code
      console.log(`  ⚠️ County field detected but no county in profile`);
      console.log(`  💡 Manual lookup: Search "${p.address?.city}, ${p.address?.state} ${p.address?.zipCode} county"`);
    ...
    // Also handles natural language labels from Local AI
    if (labelLower.includes('authorized to work') || 
        labelLower === 'whether the person is authorized to work in this country' ||
        labelLower === 'work_authorization') {
      return p.workAuth?.authorizedToWork ? 'Yes' : 'No';
    }
    if (labelLower.includes('visa sponsorship') || labelLower.includes('requires visa') || labelLower.includes('require sponsorship') ||
        labelLower === 'whether the person requires visa sponsorship' ||
        labelLower === 'visa_sponsorship') {
    ...
    // ========== CITIZENSHIP STATUS (US Citizen/PR question) ==========
    // Question: "Are you a US Citizen, Permanent Resident, or Protected Person?"
    if (labelLower === 'citizenship_status') {
      // If citizenship is USA or profile indicates US Citizen/PR, return Yes
      const citizenship = (p.personal?.citizenship || '').toLowerCase();
      if (citizenship === 'usa' || citizenship === 'us' || 
          citizenship === 'united states' || citizenship === 'american' ||
          p.workAuth?.isUSCitizenOrPR === true) {
        return 'Yes';
      }
      // Otherwise return No (they need visa sponsorship)
      return 'No';
    }
    
    // ========== VISA TYPE (J-1/J-2 question) ==========
    // Question: "Have you ever held a J-1 or J-2 exchange visitor visa?"
    if (labelLower === 'visa_type') {
      // Check if they've ever held J-1/J-2 visa
      if (p.workAuth?.hadJ1J2Visa === true) {
        return 'Yes';
      }
      const visaStatus = (p.workAuth?.visaStatus || '').toLowerCase();
      if (visaStatus.includes('j-1') || visaStatus.includes('j-2') || visaStatus.includes('j1') || visaStatus.includes('j2')) {
        return 'Yes';
      }
      return 'No';
    }
    ...
    if (labelLower === 'privacy_agreement') {
      return p.additional?.agreeToTerms ? 'Yes' : 'No';
    }
    ...
    if (labelLower === 'current_employee') {
      return 'No';  // Applicants are almost never current employees of the company they're applying to
    }
    ...
    // ========== AGE VERIFICATION (SAFETY NET) ==========
    // ALWAYS return "Yes" for adult applications - better than ATS auto-reject
    if (labelLower === 'over_18') {
      return 'Yes';  // Safety net - assume adults applying
    }
    // Also match "age_verification" from Claude classifier
    if (labelLower === 'age_verification') {
      return 'Yes';  // Safety net - assume adults applying
    }
    if (labelLower === 'criminal_history') {
      return p.additional?.criminalHistory ? 'Yes' : 'No';
    }
    // Also match "relative_at_company" from Claude classifier (without has_ prefix)
    if (labelLower === 'relative_at_company') {
      return p.additional?.hasRelativeAtCompany ? 'Yes' : 'No';
    }
    // Also match "restrictive_agreement" from Claude classifier (without has_ prefix)
    if (labelLower === 'restrictive_agreement') {
      return p.additional?.hasRestrictiveAgreement ? 'Yes' : 'No';
    }
    ...
    // These are "If yes, please explain" fields that only appear when answering Yes to compliance questions
    // Since our answers should be "No" to these questions, these fields should never appear
    // But if they do appear (due to wrong dropdown selection), skip them
    if (labelLower === 'explanation_text') {
      console.log(`      ⚠️ Explanation field detected - skipping (should not appear if compliance questions answered correctly)`);
      return null;  // Skip
    }
    
    // ========== SKILLS ==========
    if (labelLower === 'skills' || labelLower.includes('type to add skills')) {
      const technical = p.skills?.technical || [];
      const soft = p.skills?.soft || [];
      const allSkills = [...technical, ...soft];
      return allSkills.slice(0, 10);  // Top 10 skills
    }
    
    // ========== AVAILABILITY ==========
    // Notice period - convert "2 weeks" to days
    if (labelLower === 'notice_period' || 
        (labelLower.includes('notice') && labelLower.includes('period'))) {
      const availability = p.employment?.availableStartDate;
      if (availability) {
        // Convert "2 weeks" to "14" days
        if (availability.toLowerCase().includes('week')) {
          const weeks = parseInt(availability) || 2;
          return String(weeks * 7);  // Return days
        }
        // Convert "1 month" to "30" days
        if (availability.toLowerCase().includes('month')) {
          const months = parseInt(availability) || 1;
          return String(months * 30);  // Return days
        }
        // If already a number (days), return it
        if (!isNaN(parseInt(availability))) {
          return String(parseInt(availability));
        }
        // If "immediately", return 0
        if (availability.toLowerCase() === 'immediately' || 
            availability.toLowerCase() === 'now' ||
            availability.toLowerCase() === 'asap') {
          return '0';
        }
      }
      return '14';  // Default to 2 weeks (14 days)
    }
    
    // Available to start - DON'T return string, let getDateValue() handle it
    // (Removed - date detection handles this properly)
    ...
    const dateTypes = [
      'date_of_birth', 'graduation_date', 'education_start_date',
  ...
  async fillField(field, value, classificationLabel) {
    ...
    try {
      ...
      // Special handling for Workday skills field (multi-select)
      if (classificationLabel === 'skills' && Array.isArray(value) && value.length > 0) {
        console.log(`   🎯 Detected SKILLS field - using simple multi-select handler`);
        const skillsResult = await this.platform.fillSkillsField(
          this.page,
          field.selector,
          value,
          field.label
        ...
        if (skillsResult.success) {
          console.log(`   ✅ Skills: Added ${skillsResult.addedCount}/${skillsResult.totalCount}`);
          return { success: true, value: skillsResult.addedSkills.join(', ') };
        } else {
          console.log(`   ❌ Skills: Failed to add skills`);
